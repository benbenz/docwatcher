...
def call(session, url, use_proxy=False, retries=0,sleep_time=DEFAULT_SLEEP_TIME,previous_url=None):
    if use_proxy:
        proxy = pm.get_proxy()
        if proxy[0]:
            try:
...
def call_head(session, url, use_proxy=False, retries=0,sleep_time=DEFAULT_SLEEP_TIME,previous_url=None):
    if use_proxy:
        proxy = pm.get_proxy()
        if proxy[0]:
            try:
